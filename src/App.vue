  ...
  <div class="app">
    <div class="main-app">
      <div class="main-app__setting">
        <p class="main-app__setting--ip">{{ serverip }}</p>
...
export default {
  ...
  setup() {
      ...
      fetch(ipadress + '/server/status', {
        ...
        .then((data) => {
          if(data.serverStatus.status) {
            status.value = data.serverStatus.status
            serverip.value = "server ip: " + data.ip
          } else {
            status.value = data.serverStatus.status
            serverip.value = ""
          }
    ...
    return { propsPlayers, status, startServer, stopServer, timestamp, getCommands, changeSettings, serverip }
  }
}
</script>
